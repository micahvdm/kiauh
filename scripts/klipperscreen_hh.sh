...
function klipperscreen_setup() {
  ...
  status_msg "Cloning KlipperScreen Happy Hare from ${KLIPPERSCREEN_HH_REPO} ..."
  read -p "Number of gates on your MMU: " num_gates
  ...
  cd "${HOME}" || exit 1
  if ! git clone "${KLIPPERSCREEN_HH_REPO}" "${KLIPPERSCREEN_DIR}"; then
    print_error "Cloning KlipperScreen from\n ${KLIPPERSCREEN_HH_REPO}\n failed!"
    exit 1
  fi

  status_msg "Installing KlipperScreen ..."
  if "${KLIPPERSCREEN_DIR}"/happy_hare/install_ks.sh -g $num_gates; then
